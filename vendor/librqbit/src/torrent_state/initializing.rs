...
impl TorrentStateInitializing {
    ...
    fn is_fresh_storage(&self) -> bool {
        let output_folder = &self.shared.options.output_folder;
        for (idx, fi) in self.metadata.file_infos.iter().enumerate() {
            ...
            let candidate = output_folder.join(&fi.relative_filename);
    ...
    pub async fn check(&self) -> anyhow::Result<TorrentStatePaused> {
        ...
        let have_pieces = match have_pieces {
            Some(h) => h,
            None => {
                if self.is_fresh_storage() {
                    // Fast-path: brand new downloads with no on-disk data can skip full hashing.
                    let empty = BF::from_boxed_slice(
                        vec![0u8; self.metadata.lengths.piece_bitfield_bytes()].into_boxed_slice(),
                    );
                    bitv_factory
                        .store_initial_check(id, empty)
                        .await
